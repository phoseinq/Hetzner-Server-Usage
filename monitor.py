    ...
    def _ensure_file(self):
        if not self.data_file.exists():
            with open(self.data_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['server_id', 'last_critical'])
        ...
        with open(self.data_file, 'r') as f:
            ...
            for row in reader:
                if int(row['server_id']) == server_id:
                    return {'last_critical': row['last_critical']}
        return {'last_critical': ''}
    
    def update_server_status(self, server_id, critical=None):
        rows = []
        found = False
        with open(self.data_file, 'r') as f:
            reader = csv.DictReader(f)
            for row in reader:
                ...
                rows.append(row)
        if not found:
            rows.append({'server_id': server_id, 'last_critical': critical or ''})
        with open(self.data_file, 'w', newline='') as f:
            writer = csv.DictWriter(f, fieldnames=['server_id', 'last_critical'])
            writer.writeheader()
